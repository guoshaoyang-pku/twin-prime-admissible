...
<main>
  ...
  <section>
    ...
    <p>这个战场为什么本质上是<strong>优化问题</strong>？因为整条 Maynard 路线由三层互相耦合的优化组成：</p>
    <div class="two-col">
      <div class="panel">
        <h3>第一层：组合优化——选模板</h3>
      ...
      <div class="panel">
        <h3>第二层：变分优化——调权重</h3>
        <p>光有模板不够：筛法本身只能给出“平均而言素数稀疏”，要证明的是某个<strong>加权平均可以超过阈值</strong>。做法是在 (k−1) 维单纯形上放一个非负权重函数 F，把“素数出现的加权平均次数”最大化，得到变分常数 M<sub>k</sub>。</p>
  ...
  └── θ    ：解析层，越大阈值越低（素数分布输入）</pre>
    ...
    <p>关键观察：246 之后十年无人推进，不是变分层没拧干净——而是阈值 2/θ 被 θ=1/2 钉死在 4，M<sub>50</sub>=4.0043 已经贴着天花板。要再往下，必须换解析输入。这正是 Stadlmann 和 GPT-6 做的事：提高光滑模上的分布水平，阈值降到 ≈3.81，于是更小的 k=40 就够了，直径直接砍到 186。</p>
  ...
  <section>
    ...
    <div class="two-col">
      ...
      <div class="panel">
        <h3>给读者的 takeaway</h3>
        <p>AI 没有替代“决定哪扇门值得开”的判断——Polymath8b 九年前就写下了那扇门。但 AI 正在把“开门”的成本从以年计压到以天计：张益唐到 Maynard 用了数月，Polymath 用了全球协作，而 9 月 4 日一天之内出现了三个新纪录。</p>
        <p>差别不在算力，而在<strong>地图感</strong>：知道哪面墙是死的，哪扇门只是没锁。DSH 的五天没有白跑——它用 483 万个事件把一面墙的厚度量了出来。下一次，先给 agent 一张好地图。</p>
      </div>
    </div>
  </section>
