...
把 1436 个 (k,d) 下界目标（k=20..50 全覆盖，小 k 有 Lean 穷举）的完备证明
压缩为 Lean 4 可机器检查的证书。
...
| 类别 | 内容 | 状态 |
|---|---|---|
| **已严格证明（本项目）** | H(20..42) 与 H(43..50) 的下界（1436 个 (k,d) 目标，Lean 4 证书验证；k=43..50 另有 529 个 + 早期 C 穷举互证）+ 显式 SAT 见证 + 三算法交叉验证 | ✅ 机器检查 |
...
4. **1436 个定理**（`lean_theorems/`）：`H50_gt_244 : ¬ ∃ t, admissible 50 t ∧ diameter t ≤ 244`
   等，每条 `exact certValidRoot_sound ... (by native_decide)`；
5. **31 个纯内核样本**（`lean_kernel/`）：每个 k 一个边界目标用 `by decide`（纯内核归约，
   无 native_decide 信任边界）。
...
- **原始并行运行数据未随仓库发布**（约 42.4 万个结果文件、1.6 GB，见 `.gitignore`）——
  **正式审计入口是 `lean_cert/` 的 1436 个 Lean 证书与定理**；
