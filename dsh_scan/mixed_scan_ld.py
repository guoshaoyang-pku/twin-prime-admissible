#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""mixed_scan.py — 论文等价基: {(1-P1)^r · p_γ : r + deg(γ) ≤ D, γ 偶数划分}
矩阵公式 (对应论文 Lemma bfi 的幂和等价形式):
  I[(r1,α),(r2,β)] = (1+ε)^{k+deg+r1+r2} · (r1+r2)! · H(γ;k) / (k+r1+r2+deg)!
  J1 同 floatbuild 公式, s = r1+r2+2+Sa+Sb, Ba = r1!·Sa!/(r1+Sa+1)!
用法: python3 mixed_scan.py k eps_num eps_den D
"""
import sys, time, math
sys.path.insert(0, '.')
import floatbuild as fb
from floatbuild import multiset_key, split_a
import numpy as np

def H_of_ld(parts, coords, cache):
    key = (multiset_key(parts), coords)
    if key in cache:
        return cache[key]
    dp = {(): np.longdouble(1.0)}
    for v in parts:
        ndp = {}
        for state, cnt in dp.items():
            for idx, s_ in enumerate(state):
                if idx > 0 and state[idx - 1] == s_:
                    continue
                mult = state.count(s_)
                ns = list(state)
                ns[idx] = s_ + v
                ns = tuple(sorted(ns))
                ndp[ns] = ndp.get(ns, np.longdouble(0.0)) + cnt * np.longdouble(mult)
            ns = tuple(sorted(state + (v,)))
            ndp[ns] = ndp.get(ns, np.longdouble(0.0)) + cnt
        dp = ndp
    val = np.longdouble(0.0)
    for state, cnt in dp.items():
        if len(state) <= coords:
            f = math.factorial(coords) // math.factorial(coords - len(state))
            val += cnt * np.longdouble(math.prod(math.factorial(s_) for s_ in state) * f)
    cache[key] = val
    return val

def float_lambda_max_ld(I, J1):
    n = len(I)
    Ifl = np.array(I, dtype=np.longdouble)
    Jfl = np.array(J1, dtype=np.longdouble)
    d = np.longdouble(1.0) / np.sqrt(np.maximum(np.diag(Ifl), np.longdouble(1e-300)))
    Isc = Ifl * d[:, None] * d[None, :]
    Jsc = Jfl * d[:, None] * d[None, :]
    I64 = Isc.astype(np.float64)
    J64 = Jsc.astype(np.float64)
    try:
        L = np.linalg.cholesky(I64)
    except np.linalg.LinAlgError:
        I64 = I64 + 1e-12 * np.eye(n)
        L = np.linalg.cholesky(I64)
    Linv = np.linalg.solve(L, np.eye(n))
    B = Linv @ J64 @ Linv.T
    ev = np.linalg.eigvalsh(B)
    return float(ev[-1])

def gen_even_partitions(max_deg):
    """偶数 part 划分 (非增元组), 总度数 ≤ max_deg"""
    res = []
    def rec(deg_used, parts):
        res.append(tuple(parts))
        start = parts[-1] if parts else 2
        for v in range(start, max_deg - deg_used + 1, 2):
            rec(deg_used + v, parts + [v])
    rec(0, [])
    return res

def main():
    t0 = time.time()
    k = int(sys.argv[1]); en = int(sys.argv[2]); ed = int(sys.argv[3]); D = int(sys.argv[4])
    eps = en / ed
    parts = gen_even_partitions(D)
    basis = []  # (r, gamma)
    for gamma in parts:
        dg = sum(gamma)
        for r in range(0, D - dg + 1):
            basis.append((r, gamma))
    n = len(basis)
    print(f"k={k} eps={en}/{ed} D={D} mixed-basis n={n}", flush=True)
    t1 = time.time()
    H_cache = {}
    for coords in (k, k - 1):
        for gamma in parts:
            H_of_ld(list(gamma), coords, H_cache)
        for gamma in parts:
            for delta in parts:
                H_of_ld(list(sorted(gamma + delta)), coords, H_cache)
    print(f"H cache built ({time.time()-t1:.0f}s, {len(H_cache)} entries)", flush=True)
    fac = {m: math.factorial(m) for m in range(0, 2 * D + 100)}
    c1, c2 = np.longdouble(1.0) - np.longdouble(eps), np.longdouble(2.0) * np.longdouble(eps)
    one_eps = np.longdouble(1.0) + np.longdouble(eps)
    eppow = [np.longdouble(1.0)]
    for _ in range(k + 2 * D + 10):
        eppow.append(eppow[-1] * one_eps)
    G_cache = {}
    def G(s, deg):
        key = (s, deg)
        v = G_cache.get(key)
        if v is not None:
            return v
        tot = np.longdouble(0.0)
        for j in range(s + 1):
            tot += np.longdouble(math.comb(s, j)) * c1 ** np.longdouble(j) * c2 ** np.longdouble(s - j) * np.longdouble(fac[j]) / np.longdouble(fac[k - 1 + j + deg])
        G_cache[key] = tot
        return tot
    splits = {gamma: split_a(list(gamma)) for gamma in parts}
    t2 = time.time()
    I = [[np.longdouble(0.0)] * n for _ in range(n)]
    J1 = [[np.longdouble(0.0)] * n for _ in range(n)]
    for ia, (r1, alpha) in enumerate(basis):
        for ib, (r2, beta) in enumerate(basis):
            gamma = multiset_key(tuple(sorted(alpha + beta)))
            deg = sum(gamma)
            rr = r1 + r2
            I[ia][ib] = eppow[k + deg + rr] * np.longdouble(fac[rr]) * H_of_ld(list(gamma), k, H_cache) / np.longdouble(fac[k + rr + deg])
            tot = np.longdouble(0.0)
            for ca, Sa, ga in splits[alpha]:
                Ba = np.longdouble(fac[r1]) * np.longdouble(fac[Sa]) / np.longdouble(fac[r1 + Sa + 1])
                for cb, Sb, gb in splits[beta]:
                    Bb = np.longdouble(fac[r2]) * np.longdouble(fac[Sb]) / np.longdouble(fac[r2 + Sb + 1])
                    mu = multiset_key(tuple(sorted(ga + gb)))
                    dmu = sum(mu)
                    s = rr + 2 + Sa + Sb
                    tot += np.longdouble(ca * cb) * Ba * Bb * (c1 ** np.longdouble(k - 1 + dmu)) * H_of_ld(list(mu), k - 1, H_cache) * G(s, dmu)
            J1[ia][ib] = tot
    print(f"matrices built ({time.time()-t2:.0f}s)", flush=True)
    lam = float_lambda_max_ld(I, J1)
    M = k * lam
    print(f"RESULT k={k} eps={en}/{ed} D={D}: lambda={lam:.6f} M ~= {M:.6f}  (total {time.time()-t0:.0f}s)", flush=True)

if __name__ == '__main__':
    main()
